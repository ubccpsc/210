...
The rest of this reading works through three examples in this order, from the simplest to the most involved. As we go we will meet the building blocks TypeScript provides: primitive values for atomic facts, restricted values for fixed choices, objects for grouping related values, unions for distinct cases, and self-reference for recursive structure.
...
This example exercises every step. Following the analysis, the nouns give us two entities, a **playlist** and a **song** (step 1). A playlist has two distinct cases, empty or non-empty, while a song always has the same shape and so has just one case (step 2). The empty case needs no information; the non-empty case needs two things, its first song and the rest of the playlist (step 3). That last piece, the rest, is itself a playlist, so this definition is **recursive**.

### Translate into a TypeScript type

Translating this needs three building blocks we have not seen yet: an object to group a song's facts, a tagged union to capture the two cases, and self-reference for the recursion.

**Objects.** A song's facts belong together, so we group them in an **object type**, which lists named properties and their types. It helps to be careful about two words here. A *type* describes a shape; it is not itself a value. `Song` is the shape; an actual song is an **object**, a value that has that shape. There is no constructor to call: any value with the right properties, written directly as an **object literal**, is a `Song`. Each object is its own independent value, so `song1` and `song2` below are two separate songs that merely share the `Song` shape.

<details class="tooltip ts-tips">
  <summary>An object type and two of its objects</summary>
...
// song1 is one object of type Song
const song1: Song = {
  title: "Song A",
  artist: "Artist 1",
  durationSeconds: 200
...
// song2 is a second, independent object of the same type
const song2: Song = {
  title: "Song B",
  artist: "Artist 2",
  durationSeconds: 180
...
There is no `makeSong` function; an object is built directly by listing its properties. The type cannot express that a duration must be positive, so we record that constraint in a comment and rely on tests to enforce it.
...
An object type plays the role of a structure definition. Where CPSC 110 had `(define-struct song (title artist duration))` and built values with `(make-song title artist duration)`, TypeScript describes the same grouping with an object type and builds values with an object literal, with no constructor in between.

</details>

**Tagged unions.** A playlist has two cases, so we model it as a **union** of one type per case. To tell the cases apart, each carries a shared **discriminator** property, here called `kind`, set to a different constant. This is the idiomatic way to represent "one of several kinds of thing" in TypeScript: when you check the discriminator, the compiler knows which case you are in and which properties are available.
...
`EmptyPlaylist` carries no song data; `NonEmptyPlaylist` carries the first song and the rest of the playlist. Code reads `kind` to learn which case it has before touching the other properties.
...
**Recursive data.** The `rest` property has type `Playlist` again. This self-reference is what lets one type describe a playlist of any length: a non-empty playlist contains a playlist, which may itself be empty or non-empty, and so on until the empty case ends the chain.

### Write examples to check your model

With the types written, we build concrete examples. If they are easy to construct, the design fits; if they are awkward, the model is probably too complicated. These examples also become the data our tests run against later. Each playlist nests toward the empty case, reusing the `song1` and `song2` objects from above.
...
const twoTracks: Playlist = {
  kind: "songs",
  first: song1,
  rest: { kind: "songs", first: song2, rest: { kind: "empty" } }
};
```
...
Finally, look for repetition. A playlist is one instance of a more general shape: a list of any element type. If a program needed lists of several different things, we would write that shape once and let it take the element type as a **parameter**, written in angle brackets.
...
With the data defined, writing functions over it is far less open-ended than it first appears, because the structure of the code mirrors the structure of the data. The data definition hands you a template for the function:

- If the data has distinct cases, the function examines which case it has.
- If the data is recursive, the function is recursive.

This is why the modelling work pays off: a precise data definition has already done much of the design of the functions that consume it.
...
<details class="tooltip ts-tips">
  <summary>Case analysis with `if`/`else`</summary>
